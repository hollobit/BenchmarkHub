...
const DatasetCard: React.FC<DatasetCardProps> = ({ 
  ...
  const handleToggle = (e: React.MouseEvent) => {
    if (showSelection && onToggleSelect) {
      onToggleSelect(dataset.paperLink);
    }
  };
  ...
  const SelectionOverlay = () => {
    ...
    return (
      <div 
        ...
        onClick={(e) => { e.stopPropagation(); onToggleSelect?.(dataset.paperLink); }}
      >
  ...
  const GroundingSources = () => {
    if (!dataset.groundingSources || dataset.groundingSources.length === 0) return null;
    return (
      <div className="mt-4 pt-4 border-t border-slate-100">
        <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Search Grounding Sources</h4>
        <div className="flex flex-wrap gap-2">
          {dataset.groundingSources.slice(0, 4).map((source, i) => (
            <a 
              key={i} 
              href={source.uri} 
              target="_blank" 
              rel="noopener noreferrer"
              className="text-[10px] bg-indigo-50 text-indigo-700 px-2 py-1 rounded hover:bg-indigo-100 transition-colors truncate max-w-[150px] border border-indigo-100"
              title={source.title}
