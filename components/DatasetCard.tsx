...
const DatasetCard: React.FC<DatasetCardProps> = ({ dataset, onSave, onRemove, isSaved, viewMode = 'grid' }) => {
  const handleAction = () => {
    if (isSaved) {
      onRemove?.(dataset.id);
    } else {
  ...
  if (viewMode === 'list') {
    return (
      <div className="bg-white border border-slate-200 rounded-lg p-4 flex flex-col md:flex-row md:items-center gap-4 hover:bg-slate-50 transition-colors">
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow p-6 flex flex-col h-full">
